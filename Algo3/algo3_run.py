...
4. Replaced Unix-only signal.SIGALRM with cross-platform threading.Timer
5. Added clear "Timeout reached" message in CSV log
...
def solve_single_puzzle(puzzle_string: str, show_output: bool = True) -> Tuple:
    ...
    try:
        ...
        if timeout_occurred:
            raise TimeoutError("Timeout during AC-3")
        ...
        if not ac3_result:
            ...
            if timeout_occurred:
                raise TimeoutError("Timeout during Backtracking")
            if result:
...
def solve_all_puzzles(filenames: List[str]) -> None:
    ...
    if all_results:
        ...
        # ===========================================================
        # Statistics by Difficulty (only once)
        # ===========================================================
        print(f"\n{'='*70}")
